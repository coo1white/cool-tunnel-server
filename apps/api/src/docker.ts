...
export function mapContainerState(state: DockerContainerState | null): {
  status: ServiceStatus;
  detail: string;
} {
  if (!state?.Status) return { status: "stopped", detail: "Container not found." };
  const health = state.Health?.Status;
  switch (state.Status) {
    case "running":
      if (health === "unhealthy")
        return { status: "degraded", detail: "Running but health check failing." };
      if (health === "starting")
        return { status: "degraded", detail: "Running; health check starting." };
      return {
        status: "running",
        detail: health === "healthy" ? "Running (healthy)." : "Running.",
      };
    case "restarting":
