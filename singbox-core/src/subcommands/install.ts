...
import { chmodSync, mkdirSync, mkdtempSync, renameSync, statSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { flagValue } from "../util/argv.ts";
import { sha256Hex } from "../util/sha256.ts";
import { currentAssetKey, SINGBOX_UPSTREAM, SINGBOX_UPSTREAM_TAG } from "../version.ts";
...
export async function runInstall(argv: readonly string[]): Promise<number> {
  ...
  if (!asset) {
    throw new Error(`singbox.upstream.json missing pin for "${assetKey}" — refresh pin file`);
  }
  ...
  if (args.verify) {
    ...
    if (out.status !== 0) {
      throw new Error(`post-install verify failed: \`sing-box version\` exited ${out.status}`);
    }
