...
v0.5.2 replaced the PHP/Laravel control plane with the Better-T-Stack
monorepo. The active runtime has no MariaDB, Redis, Composer, or PHP
worker process. The control-plane database is SQLite at
`./data/admin/admin.sqlite` on the host, bind-mounted into admin-api as
`/data/admin/admin.sqlite`.

`ct admin migrate` brings the SQLite schema to the required version.
There is no PHP/Laravel data importer: the legacy `legacy_*` staging
path was removed once the rebuild was complete.
