...
#
# caddy-l4 plugin baked in via docker/caddy/Dockerfile. Two routes —
# the first matches the proxy SNI exactly and TCP-proxies raw bytes
# to ct-naive; the second is the unconditional fallback for anything
# else (panel.*, or any random scanner) and forwards to the inner
# HTTPS server on 127.0.0.1:8443 where Caddy's HTTP app multiplexes
# by SNI/Host as normal.
#
# Why raw TCP for naive: we want naive to terminate TLS itself (so
# the wire format matches what the macOS client speaks end-to-end
# with NO intermediary parsing the TLS stream). Caddy never sees the
# decrypted bytes; it's a transparent SNI splitter.
layer4 {
    :443 {
        @naive_sni tls {
            sni {{ .Domain }}
        ...
        route @naive_sni {
            proxy {
                upstream tcp/ct-naive:443
            }
        }

        # Catch-all: panel.* SNI + every probe. Forward raw bytes to
        # the inner Caddy HTTPS listener on 127.0.0.1:8443. The inner
        # listener has the panel cert (and the dummy naive cert-
        # acquisition block); SNI mismatch falls through to Caddy's
        # default-host behaviour, which for an unknown SNI returns
        # the cover-site default site (currently empty 404).
        route {
            proxy {
