...
export async function runRestore(backupPath: string): Promise<number> {
  ...
  if (await stackIsRunning())
    die("stack is currently running; refusing to restore over it", "docker compose down -v");
  ...
  if (stale.length > 0)
    die(
      ...
      `remove old stack first: docker compose down -v\nstale volumes: ${stale.join(", ")}`,
    );
  ...
  try {
    ...
    if (!bundle.ok)
      die(
        ...
        "run ./scripts/fetch_image_bundle.sh, then retry restore",
      );
    ...
    const up = await capture(
      $`docker compose up -d --no-build --pull never --remove-orphans admin-api admin-web singbox caddy`,
    );
