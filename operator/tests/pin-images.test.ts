...
test("fromLineRe matches a bare FROM <image> line", () => {
    const re = fromLineRe("caddy:2.8.4-alpine");
    expect(re.test("FROM caddy:2.8.4-alpine")).toBe(true);
    expect(re.test("FROM caddy:2.8.4-alpine ")).toBe(true);
});
...
test("fromLineRe matches an already-pinned line", () => {
    const re = fromLineRe("caddy:2.8.4-alpine");
    expect(re.test("FROM caddy:2.8.4-alpine@sha256:deadbeefcafe")).toBe(true);
    expect(re.test("FROM caddy:2.8.4-alpine@sha256:abc123 AS web")).toBe(true);
});

test("fromLineRe does NOT match a different image with a similar prefix", () => {
    const re = fromLineRe("alpine:3.20");
    // Naive substring matching would let "alpine:3.20-cuda" through;
    // the trailing (@sha256|space|EOL) guard prevents it.
    expect(re.test("FROM alpine:3.20-cuda")).toBe(false);
});

test("fromLineRe does NOT match non-FROM lines", () => {
    const re = fromLineRe("caddy:2.8.4-alpine");
    expect(re.test("# FROM caddy:2.8.4-alpine")).toBe(false);
    expect(re.test("ARG IMAGE=caddy:2.8.4-alpine")).toBe(false);
    expect(re.test("RUN echo caddy:2.8.4-alpine")).toBe(false);
});

test("rewriteDockerfile pins a single FROM line", () => {
    const body = "FROM caddy:2.8.4-alpine\nCMD [\"caddy\"]\n";
    const r = rewriteDockerfile(body, "caddy:2.8.4-alpine", "sha256:abc");
    expect(r.changedLines).toBe(1);
    expect(r.content).toBe("FROM caddy:2.8.4-alpine@sha256:abc\nCMD [\"caddy\"]\n");
});
...
test("rewriteDockerfile replaces a previously-pinned digest", () => {
    ...
    const body = `FROM caddy:2.8.4-alpine@sha256:${oldHex} AS web\n`;
    const r = rewriteDockerfile(body, "caddy:2.8.4-alpine", `sha256:${newHex}`);
    expect(r.changedLines).toBe(1);
    expect(r.content).toBe(`FROM caddy:2.8.4-alpine@sha256:${newHex} AS web\n`);
});
...
test("rewriteDockerfile does not touch a similarly-named image", () => {
    const body = "FROM alpine:3.20-cuda\n";
    const r = rewriteDockerfile(body, "alpine:3.20", "sha256:abc");
    expect(r.changedLines).toBe(0);
    expect(r.content).toBe(body);
});
