...
async function migrateSqlite(): Promise<void> {
  ...
  try {
    ...
    if (!store.hasOwner())
      warn("no owner exists yet; run `ct admin bootstrap` after services start");
  } finally {
    db.close();
  }
}
...
async function renderInitialConfig(): Promise<void> {
  ...
  const caddy = await capture(
    $`docker compose run --rm --no-deps admin-api bun -e ${renderScript("render-caddyfile")}`,
  );
  if (!caddy.ok) die("Caddyfile render failed", caddy.stderr.split("\n")[0] ?? "");
  const singbox = await capture(
    $`docker compose run --rm --no-deps admin-api bun -e ${renderScript("render-singbox")}`,
  );
...
async function startStack(): Promise<void> {
  ...
  const up = await capture(
    $`docker compose up -d --no-build --pull never --remove-orphans admin-api admin-web singbox caddy docker-proxy`,
  );
