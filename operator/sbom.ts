...
// operator/sbom.ts — generate CycloneDX SBOMs for Rust, TypeScript, and Docker.
//
...
//   - cyclonedx/cdxgen for the Bun/TypeScript monorepo and Docker images
//
...
export function buildCombinedManifest(opts: {
  ...
  return {
    ...
    "x-references": [
      ...
      "cool-tunnel-server-admin-web.cdx.json",
    ],
  };
}
...
async function main(): Promise<number> {
  ...
  if (!which("cargo-cyclonedx")) {
    ...
    if (inst.exitCode !== 0) {
      die(
        "cargo install cargo-cyclonedx failed",
  ...
  step("Generating TypeScript SBOM (cdxgen)");
  ...
  if (!cdxgen) {
    warn("no cdxgen / bunx / npx on PATH; skipping TypeScript SBOM");
  } else {
    const r =
      await $`${cdxgen} -t js -o sbom/typescript.cdx.json --spec-version 1.5 --no-recurse`.nothrow();
    if (r.exitCode !== 0) {
      warn("cdxgen failed on TypeScript workspace (TypeScript SBOM skipped)");
    } else {
      ok("wrote sbom/typescript.cdx.json");
    }
  }
  ...
  if (which("docker") && cdxgen) {
    ...
    for (const image of DOCKER_IMAGES) {
      ...
      const r =
        await $`${cdxgen} -t docker -o sbom/${image}.cdx.json --spec-version 1.5 ${image}:latest`
          .nothrow()
          .quiet();
      if (r.exitCode !== 0) {
  ...
  if (!version) {
    die(
      "could not extract version from core/Cargo.toml",
