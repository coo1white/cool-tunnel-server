...
export async function measureDiskSpace(): Promise<DiskSpaceMeasurement> {
  ...
  const dockerRoot =
    dockerInfo.ok && dockerInfo.stdout.trim() ? dockerInfo.stdout.trim() : "/var/lib/docker";
...
export async function checkStackUp(services: readonly string[]): Promise<StackUpResult> {
  if (services.length === 0) {
    return {
      ok: true,
      summary: "preflight_stack_up: no services specified, skipping",
      missing: [],
      runningCount: 0,
    };
  }
  ...
  const r = await capture($`docker compose ps --status running --status restarting --services`);
...
export type DockerDaemonIpv4OnlyMerge =
  | {
      ...
      readonly dockerIpv4Only: boolean;
    }
...
export function classifyIpv4OnlyPreflight(input: {
  ...
  if (!input.fixResult) {
    if (alreadyIpv4Only) {
      return {
        action: "ok",
        detail: "IPv4-only already enforced (IPv4-only sysctl + Docker daemon config)",
      };
    }
    if (!input.canDetect) {
      return {
        action: "warn",
        detail:
          "IPv4-only preflight did not run; `curl` is missing and Docker/Rust may still use non-IPv4 routes.",
      };
    }
    return {
      action: "warn",
      detail: "IPv4-only preflight did not run; Docker/Rust may still use non-IPv4 routes.",
    };
  }
  ...
  if (input.rustStaticIpv4Ok === false) {
    return {
      action: "warn",
      detail:
        "IPv4-only enforced, but static.rust-lang.org is not reachable over IPv4. Check VPS outbound HTTPS/DNS, then rerun ./ct update.",
    };
  }
