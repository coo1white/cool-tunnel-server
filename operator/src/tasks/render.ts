...
): { target: Target; passthrough: string[] } | string {
...
export class RenderTask implements Task {
  ...
  async run(ctx: RunContext): Promise<TaskResult> {
    ...
    const r = await capture(
      $`docker compose run --rm --no-deps admin-api bun -e ${renderScript(action)}`,
    );
    ...
    if (target === "singbox") {
      const file = await capture(
        $`docker compose run --rm --no-deps --entrypoint sh admin-api -c ${"test -s /data/config/singbox.json"}`,
      );
      if (!file.ok) {
        return {
          ok: false,
