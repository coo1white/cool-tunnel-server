...
async function preflightStackHealthy(): Promise<{ ok: true } | { ok: false; reason: string }> {
    ...
    const up = await capture($`docker compose exec -T admin-api bun -e ${"const r=await fetch('http://127.0.0.1:9000/up'); process.exit(r.ok ? 0 : 1)"}`);
    if (!up.ok) {
        return { ok: false, reason: "stack pre-flight: admin API /up is not healthy; run 'ct doctor' first" };
    }
    return { ok: true };
}
